...
function handleAnalyzeExam(body) {
  // 2026-09-17: 학부모 문자용 분석(range·analysis·supplement 등) 추가. 기존 필드는 옛 화면을 위해 그대로 유지
  const ctx = [];
  if (body.studentName) ctx.push('학생 이름: ' + String(body.studentName).slice(0, 20));
  if (body.grade) ctx.push('학년: ' + String(body.grade).slice(0, 10));
  if (body.date) ctx.push('시험 날짜: ' + String(body.date).slice(0, 10));
  const prompt = '다음은 수학학원 데일리 테스트를 선생님이 채점한 시험지(또는 채점 결과지) 사진이야 (여러 장이면 같은 시험의 연속된 페이지야).\n' +
    (ctx.length ? ctx.join(' / ') + '\n' : '') +
    '\n[채점 표시 읽는 법]\n' +
    '- 문항 번호 위의 동그라미(○)·V체크는 정답, 빗금(/)·X·별표·세모는 오답으로 봐. 빨간 펜 표시를 우선으로 봐.\n' +
    '- 채점 결과지(표 형식)라면 O/X 칸을 그대로 읽어.\n' +
    '- 표시가 애매한 문항은 추측하지 말고 uncertainNumbers에 넣어.\n' +
    '- 틀린 문항은 문제 내용을 보고 어떤 유형인지(예: "분모가 다른 분수의 덧셈", "일차방정식 활용-거리·속력") 짧게 판단해.\n' +
    '\n아래 JSON 형식으로만 답변해줘. 다른 설명이나 인사말, 코드블록 표시(```) 없이 순수 JSON만 출력해줘.\n' +
    '{\n' +
    '  "examName": "시험명 (사진에서 유추, 모르면 빈 문자열)",\n' +
    '  "range": "오늘 시험범위를 교과 단원명으로 짧게 (예: 중2 일차함수 - 기울기와 y절편)",\n' +
    '  "totalQuestions": 전체문항수(숫자),\n' +
    '  "correctCount": 맞은개수(숫자),\n' +
    '  "score": 100점 만점 기준 점수(숫자),\n' +
    '  "wrongNumbers": [틀린 문항 번호들의 배열(숫자)],\n' +
    '  "uncertainNumbers": [채점 표시가 애매한 문항 번호 배열, 없으면 []],\n' +
    '  "unit": "시험범위 단원 (range와 같아도 됨)",\n' +
    '  "strengths": "잘한 부분 한 구절 (예: 기울기 구하기는 모두 정확)",\n' +
    '  "weakTypes": [{"numbers": [틀린 번호들], "type": "약한 유형 이름"}],\n' +
    '  "causeGuess": "예상 오답 원인, 다음 중 하나로: 단순 계산실수 / 개념 이해 부족 / 응용력 부족 / 시간 부족",\n' +
    '  "analysis": "학부모 문자용 분석 1~2문장. 잘한 점 → 약했던 유형(번호 포함)과 그 이유 순서. 예: 기울기 구하기는 안정적이었지만, y절편을 이용해 식을 세우는 문제(12·15번)에서 부호 실수가 있었습니다.",\n' +
    '  "supplement": "학부모 문자용 보완 내용 1문장. 약한 유형에 맞춰 수업에서 어떻게 채웠는지 구체적으로. 예: 틀린 문제는 수업 중 오답 풀이로 부호 처리 과정을 다시 짚었고, 같은 유형 3문제를 더 풀며 확인했습니다. 다 맞았으면 다음 단계 준비 내용으로.",\n' +
    '  "teacherComment": "학부모님께 전달할 수 있는 자연스러운 한국어 문장 3~4문장. 오늘 다룬 단원/내용, 잘한 점과 부족했던 점, 앞으로의 보완 계획을 순서대로 포함. 따뜻한 선생님 말투로 작성."\n' +
    '}\n' +
    '\n[문장 규칙] 학원 수학 선생님이 학부모님께 보내는 존댓말(~했습니다). 과장·감탄 남발 금지, 담백하고 구체적으로. 학생을 탓하지 말 것. 문제 문장을 그대로 베끼지 말 것. 이모지 쓰지 말 것. 학생 이름은 analysis·supplement에 넣지 말 것.';
